...
class View:
    carrinho = Carrinho("dados_estoque.json", "dados_pedidos.json")
    ...
    def produto_inserir(descricao, preco, estoque, id_categoria):
        if preco <= 0 or estoque < 0:
            return "Preço deve ser positivo e estoque não pode ser negativo."
    ...
    @staticmethod
    def carrinho_adicionar_produto(cliente_id, produto_id, quantidade):
        produto = Produtos.listar_id(produto_id)
        if produto is None or quantidade > produto.estoque:
            return "Produto ou estoque indisponível."
        
        View.carrinho.adicionar_produto(produto, quantidade, cliente_id)
        return "Produto adicionado ao carrinho com sucesso."
    
    @staticmethod
    def carrinho_fechar_pedido(cliente_id):
        resultado = View.carrinho.fechar_pedido(cliente_id)
        return resultado
    
    @staticmethod
    def carrinho_listar_pedidos(cliente_id):
        pedidos = View.carrinho.ver_pedidos(cliente_id)
        return pedidos
