...
class UI:
    ...
    def menu_cliente():
        print("1 - Listar Produtos, 2 - Adicionar   o Carrinho, 3 - Fechar Pedido, 4 - Ver Meus Pedidos")
        print("0 - Sair, 99 - Fim")
        op = int(input("\nInforme uma opção: "))
        if op == 0: UI.sair_do_sistema()
    ...
    def cliente_adicionar_produto(cls):
         ...
         resultado = View.carrinho_adicionar_produto(cls.cliente_id, produto_id, quantidade)
         print(resultado)
        
    @classmethod
    def cliente_fechar_pedido(cls):
        resultado = View.carrinho_fechar_pedido(cls.cliente_id)
        print(resultado)

    @classmethod
    def cliente_meus_pedidos(cliente_id):
        pedidos = View.carrinho_listar_pedidos(cliente_id)
        if len(pedidos) == 0:
